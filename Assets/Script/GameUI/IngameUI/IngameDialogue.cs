public class IngameDialogue {
    
    //Zone start and ends
    public const int LEVEL_1_START = 10;
    public const int LEVEL_1_END = 11;

    //Event dialogues
    public const int TUTORIAL_STRAIGHT = -1;
    public const int TUTORIAL_CURVE_FAIL = -2;
    public const int TUTORIAL_CURVE = -3;
    public const int TUTORIAL_FRAGMENT = -4;
    public const int TUTORIAL_HARD_CURVE = -5;
    public const int TUTORIAL_SPLIT = -6;
    public const int TUTORIAL_PROTOCOL = -7;
    public const int TUTORIAL_END = -8;

}
